package dataForTests;

public class URLsAndAPIs {
    public static final String MAIN_HOST = "http://qa-scooter.praktikum-services.ru";
    public static final String CREATE_COURIER = "/api/v1/courier";
    public static final String DELETE_COURIER = "/api/v1/courier/";
    public static final String GET_ORDERS = "/api/v1/orders";
    public static final String LOG_IN = "/api/v1/courier/login";

}
